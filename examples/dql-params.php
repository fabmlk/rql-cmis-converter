...
$rql = '(not(like(document,tot*))|atdepth(1234,5)|eq(identity(group),toto)|(name<>3&d>=string:4&eq(name,-3)))&u!=5&not(or(u=null(),between(i,1,10)))&z=1&(a==2|b<-3|out(c,(2,float:3)))&select(a,sum(b),max(i),identity(a,a))&sort(+a,-b)&limit(1,2)&groupby(type)';
echo $rql . PHP_EOL;
